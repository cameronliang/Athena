...
//----------------------------------------------------------------------------------------
// VolCenterWidthX functions: return physical width in X-dir at (i,j,k) cell-face

void Coordinates::VolCenterWidth1(const int k, const int j, const int il, const int iu,
                               AthenaArray<Real> &dx1)
{
#pragma simd
  for (int i=il; i<=iu; ++i){
    // dv1v(i)=x1v(i+1)-x1v(i)
    dx1(i) = dx1v(i-1);
  }
  return;
}

void Coordinates::VolCenterWidth2(const int k, const int j, const int il, const int iu,
                               AthenaArray<Real> &dx2)
{
#pragma simd
  for (int i=il; i<=iu; ++i){
...
void Coordinates::VolCenterWidth3(const int k, const int j, const int il, const int iu,
                               AthenaArray<Real> &dx3)
{
#pragma simd
  for (int i=il; i<=iu; ++i){
