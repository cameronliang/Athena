...
void SphericalPolar::VolCenterWidth2(const int k, const int j, const int il, const int iu,
                               AthenaArray<Real> &dx2)
{
#pragma simd
  for (int i=il; i<=iu; ++i){
    dx2(i) = x1v(i)*dx2v(j-1);
  }
  return;
}

void SphericalPolar::VolCenterWidth3(const int k, const int j, const int il, const int iu,
                               AthenaArray<Real> &dx3)
{
#pragma simd
  for (int i=il; i<=iu; ++i){
    dx3(i) = x1v(i)*fabs(sin(x2v(j)))*dx3v(k-1);
  }
  return;
}
