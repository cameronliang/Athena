...
void NTCIntegrator::AddSourceTerms(MeshBlock *pmb, const Real dt, 
     AthenaArray<Real> &u, AthenaArray<Real> &u_tc, const int step)
{
    ...
    for(int j=js; j<=je; ++j){
      ...
      for(int i=is; i<=ie; ++i){
        ...
        // rotate the vectors to oriante to the B direction
         ...
         Real kappa_x = ptc->ntc_kappa(0,k,j,i);
         Real kappa_y = ptc->ntc_kappa(1,k,j,i);
         Real kappa_z = ptc->ntc_kappa(2,k,j,i);
