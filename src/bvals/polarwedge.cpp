...
void PolarWedgeInnerX2(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,
     FaceField &b, AthenaArray<Real> &u_cr, AthenaArray<Real> &u_ntc, Real time, 
     Real dt, int is, int ie, int js, int je, int ks, int ke)
{
...
#pragma simd
  ...
  if(NEW_TH_CON_ENABLED){
    for (int n=1; n<=(NTC); ++n) {
      Real sign = flip_across_pole_ntc[n] ? -1.0 : 1.0;
      for (int k=ks; k<=ke; ++k) {
        for (int j=1; j<=(NGHOST); ++j) {
  #pragma simd
          for (int i=is; i<=ie; ++i) {
            u_ntc(n,k,js-j,i) = sign * u_ntc(n,k,js+j-1,i);
          }
        }
      }
    }
...
#pragma simd
    ...
    sign = flip_across_pole_hydro[IB3] ? -1.0 : 1.0;
    for (int k=ks; k<=ke+1; ++k) {
    for (int j=1; j<=(NGHOST); ++j) {
#pragma simd
      for (int i=is; i<=ie; ++i) {
...
#pragma simd
          for (int i=is; i<=ie; ++i) {
            u_ntc(n,k,(je+j),i) = sign * u_ntc(n,k,je-j+1,i);
          }
        }
      }
    }
