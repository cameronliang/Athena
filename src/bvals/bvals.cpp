...
// Set BC functions for each of the 6 boundaries in turn ---------------------------------
  ...
  if (pmb->block_bcs[INNER_X2] == POLAR_BNDRY || pmb->block_bcs[INNER_X2] == POLAR_BNDRY_WEDGE
     || pmb->block_rad_bcs[INNER_X2] == POLAR_BNDRY || 
        pmb->block_rad_bcs[INNER_X2] == POLAR_BNDRY_WEDGE) {
  ...
  if (pmb->block_bcs[OUTER_X2] == POLAR_BNDRY || pmb->block_bcs[OUTER_X2] == POLAR_BNDRY_WEDGE
    || pmb->block_rad_bcs[OUTER_X2] == POLAR_BNDRY || 
       pmb->block_rad_bcs[OUTER_X2] == POLAR_BNDRY_WEDGE) {
    if(pmb->pmy_mesh->nrbx3>1 && pmb->pmy_mesh->nrbx3%2!=0) {
      std::stringstream msg;
      msg << "### FATAL ERROR in BoundaryValues constructor" << std::endl
          << "Number of MeshBlocks around the pole must be 1 or even." << std::endl;
  ...
  for(int n=0;n<pmb->pmy_mesh->maxneighbor_;n++) {
    ...
    if(RADIATION_ENABLED) nvar += pmb->prad->n_fre_ang;
    // CR u+CR 
    if(CR_ENABLED) nvar += (NCR);
    if(NEW_TH_CON_ENABLED) nvar += (NTC);
  ...
  if(pmb->pmy_mesh->multilevel==true) { // SMR or AMR
    ...
    int size[6], im, jm, km;
    //flux correction buffer needs to hold both hydro and rad
    int nvar=NHYDRO;
    if(RADIATION_ENABLED)
 ...
 /* single CPU in the azimuthal direction with the polar boundary*/
  ...
  if(RADIATION_ENABLED){
    if(pmb->loc.level == pmb->pmy_mesh->root_level &&
       pmb->pmy_mesh->nrbx3 == 1 &&
       (pmb->block_rad_bcs[INNER_X2]==POLAR_BNDRY||pmb->block_rad_bcs[OUTER_X2]==POLAR_BNDRY||
        pmb->block_rad_bcs[INNER_X2]==POLAR_BNDRY_WEDGE||
        pmb->block_rad_bcs[OUTER_X2]==POLAR_BNDRY_WEDGE))
...
void BoundaryValues::CheckBoundary(void)
{
  ...
  if(RADIATION_ENABLED){
    if(pmb->block_rad_bcs[i]==USER_BNDRY) {
...
void BoundaryValues::ApplyPhysicalBoundaries(AthenaArray<Real> &pdst,
          AthenaArray<Real> &cdst, FaceField &bfdst, AthenaArray<Real> &bcdst,
                           AthenaArray<Real> &crdst, AthenaArray<Real> &ntcdst,  
                                                const Real time, const Real dt)
{
  MeshBlock *pmb=pmy_block_;
  ...
  if (BoundaryFunction_[INNER_X1] != NULL) {
    BoundaryFunction_[INNER_X1](pmb, pco, pdst, bfdst, crdst, ntcdst, time, dt,
  ...
  if (BoundaryFunction_[OUTER_X1] != NULL) {
    BoundaryFunction_[OUTER_X1](pmb, pco, pdst, bfdst, crdst, ntcdst, time, dt,
                                pmb->is, pmb->ie, bjs, bje, bks, bke);
    if(MAGNETIC_FIELDS_ENABLED) {
      pmb->pfield->CalculateCellCenteredField(bfdst, bcdst, pco,
  ...
  if(pmb->block_size.nx2>1) { // 2D or 3D
    ...
    if (BoundaryFunction_[INNER_X2] != NULL) {
      BoundaryFunction_[INNER_X2](pmb, pco, pdst, bfdst, crdst, ntcdst, time, dt,
    ...
    if (BoundaryFunction_[OUTER_X2] != NULL) {
      BoundaryFunction_[OUTER_X2](pmb, pco, pdst, bfdst, crdst, ntcdst, time, dt,
                                  bis, bie, pmb->js, pmb->je, bks, bke);
      if(MAGNETIC_FIELDS_ENABLED) {
        pmb->pfield->CalculateCellCenteredField(bfdst, bcdst, pco,
  ...
  if(pmb->block_size.nx3>1) { // 3D
    ...
    if (BoundaryFunction_[INNER_X3] != NULL) {
      BoundaryFunction_[INNER_X3](pmb, pco, pdst, bfdst, crdst, ntcdst, time, dt,
    ...
    if (BoundaryFunction_[OUTER_X3] != NULL) {
      BoundaryFunction_[OUTER_X3](pmb, pco, pdst, bfdst, crdst, ntcdst, time, dt,
                                  bis, bie, bjs, bje, pmb->ks, pmb->ke);
      if(MAGNETIC_FIELDS_ENABLED) {
        pmb->pfield->CalculateCellCenteredField(bfdst, bcdst, pco,
             ...
             const Real time, const Real dt)
{
  ...
  if (RadBoundaryFunction_[INNER_X1] != NULL) {
    RadBoundaryFunction_[INNER_X1](pmb, pco, pdst, time, dt, pmb->is, pmb->ie, 
                                                  bjs, bje, bks, bke);
  }

  // Apply boundary function on outer-x1
  if (RadBoundaryFunction_[OUTER_X1] != NULL) {
    RadBoundaryFunction_[OUTER_X1](pmb, pco, pdst, time, dt, pmb->is, pmb->ie, 
                                                  bjs, bje, bks, bke);
  }
  ...
  if(pmb->block_size.nx2>1) { // 2D or 3D
    ...
    if (RadBoundaryFunction_[INNER_X2] != NULL) {
      RadBoundaryFunction_[INNER_X2](pmb, pco, pdst,time, dt, bis, bie, 
                                            pmb->js, pmb->je, bks, bke);
    }
    
    // Apply boundary function on outer-x2
    if (RadBoundaryFunction_[OUTER_X2] != NULL) {
      RadBoundaryFunction_[OUTER_X2](pmb, pco, pdst, time, dt, bis,bie, 
                                            pmb->js, pmb->je, bks, bke);
  ...
  if(pmb->block_size.nx3>1) { // 3D
    ...
    if (RadBoundaryFunction_[INNER_X3] != NULL) {
      RadBoundaryFunction_[INNER_X3](pmb, pco, pdst, time, dt, bis,bie,bjs,bje, 
                                                     pmb->ks, pmb->ke);
    }

    // Apply boundary function on outer-x3
    if (RadBoundaryFunction_[OUTER_X3] != NULL) {
      RadBoundaryFunction_[OUTER_X3](pmb, pco, pdst,time, dt, bis,bie,bjs,bje, 
                                                     pmb->ks, pmb->ke);
    }
...
//           AthenaArray<Real> &rdst, const Real time, const Real dt)
//  \brief Prolongate the level boundary using the coarse data

void BoundaryValues::ProlongateBoundaries(AthenaArray<Real> &pdst,
     AthenaArray<Real> &cdst, FaceField &bfdst, AthenaArray<Real> &bcdst,
     AthenaArray<Real> &rdst, AthenaArray<Real> &crdst, AthenaArray<Real> &ntcdst,
     const Real time, const Real dt)
{
  MeshBlock *pmb=pmy_block_;
  ...
  for(int n=0; n<pmb->nneighbor; n++) {
      ...
      for(int nj=njs; nj<=nje; nj++) {
        for(int ni=nis; ni<=nie; ni++) {
          ...
          pmb->pmr->RestrictCellCenteredValues(cdst, pmr->coarse_cons_, 0, NHYDRO-1,
                                               ris, rie, rjs, rje, rks, rke, HYDRO_CONS);
          //restrict radiation
          if(RADIATION_ENABLED)
            pmb->pmr->RestrictCellCenteredValues(rdst, pmr->coarse_ir_, 0, 
                              pmb->prad->n_fre_ang-1, ris, rie, rjs, rje, rks, rke, RAD);

          if(CR_ENABLED){
            pmb->pmr->RestrictCellCenteredValues(crdst, pmr->coarse_ucr_, 0, NCR-1,
                                               ris, rie, rjs, rje, rks, rke, HYDRO_CONS);
          }

          if(NEW_TH_CON_ENABLED){
            pmb->pmr->RestrictCellCenteredValues(ntcdst, pmr->coarse_untc_, 1, NTC,
                                               ris, rie, rjs, rje, rks, rke, HYDRO_CONS);
          }
          ...
          if (MAGNETIC_FIELDS_ENABLED) {
            ...
            if(pmb->block_size.nx2 > 1) {
              ...
              pmr->RestrictFieldX2(bfdst.x2f, pmr->coarse_b_.x2f,
            ...
            if(pmb->block_size.nx3 > 1) {
              ...
              pmr->RestrictFieldX3(bfdst.x3f, pmr->coarse_b_.x3f,
    ...
    if(nb.ox1==0) {
      if(BoundaryFunction_[INNER_X1]!=NULL) {
        BoundaryFunction_[INNER_X1](pmb, pmr->pcoarsec, pmr->coarse_prim_,
                pmr->coarse_b_, pmr->coarse_ucr_, pmr->coarse_untc_, time, dt, 
      ...
      if(RADIATION_ENABLED){
        ...
        if(RadBoundaryFunction_[INNER_X1]!=NULL) {
          RadBoundaryFunction_[INNER_X1](pmb, pmr->pcoarsec, pmr->coarse_ir_,
                                time, dt, pmb->cis, pmb->cie, sj, ej, sk, ek);
        }
        if(RadBoundaryFunction_[OUTER_X1]!=NULL) {
          RadBoundaryFunction_[OUTER_X1](pmb, pmr->pcoarsec, pmr->coarse_ir_,
                                time, dt, pmb->cis, pmb->cie, sj, ej, sk, ek);
        }
    ...
    if(nb.ox2==0 && pmb->block_size.nx2 > 1) {
      ...
      if(RADIATION_ENABLED){
        ...
        if(RadBoundaryFunction_[INNER_X2]!=NULL) {
          RadBoundaryFunction_[INNER_X2](pmb, pmr->pcoarsec, pmr->coarse_ir_,
                                time, dt, si, ei, pmb->cjs, pmb->cje, sk, ek);
        }
        if(RadBoundaryFunction_[OUTER_X2]!=NULL) {
          RadBoundaryFunction_[OUTER_X2](pmb, pmr->pcoarsec, pmr->coarse_ir_,
                                time, dt, si, ei, pmb->cjs, pmb->cje, sk, ek);
        }
    ...
    if(nb.ox3==0 && pmb->block_size.nx3 > 1) {
      ...
      if(RADIATION_ENABLED){
        if(RadBoundaryFunction_[INNER_X3]!=NULL) {
          RadBoundaryFunction_[INNER_X3](pmb, pmr->pcoarsec, pmr->coarse_ir_,
                                time, dt, si, ei, sj, ej, pmb->cks, pmb->cke);
        }
        if(RadBoundaryFunction_[OUTER_X3]!=NULL) {
          RadBoundaryFunction_[OUTER_X3](pmb, pmr->pcoarsec, pmr->coarse_ir_,
                                time, dt, si, ei, sj, ej, pmb->cks, pmb->cke);
        }
      }// End Radiation
    ...
    pmr->ProlongateCellCenteredValues(pmr->coarse_prim_, pdst, 0, NHYDRO-1,
                                      si, ei, sj, ej, sk, ek,HYDRO_PRIM);
    if(RADIATION_ENABLED){
      pmr->ProlongateCellCenteredValues(pmr->coarse_ir_, rdst, 0, 
                            pmb->prad->n_fre_ang-1, si, ei, sj, ej, sk, ek, RAD);
    }
    if(CR_ENABLED){
      pmr->ProlongateCellCenteredValues(pmr->coarse_ucr_, crdst, 0, NCR-1,
                                      si, ei, sj, ej, sk, ek,HYDRO_PRIM);      
    }
    if(NEW_TH_CON_ENABLED){
      pmr->ProlongateCellCenteredValues(pmr->coarse_untc_, ntcdst, 1, NTC,
                                      si, ei, sj, ej, sk, ek,HYDRO_PRIM);      
    }
