                      ...
                      AthenaArray<Real> &src_ntc, enum CCBoundaryType type)
{
  ...
  AthenaArray<Real> buf_rad, buf_cr, buf_ntc; 
  // buf pointer to radiation, cosmic rays and thermal conduction
  ...
  for(int n=0; n<pmb->nneighbor; n++) {
    ...
    if(CR_ENABLED){
      crns=0, crne=NCR-1;
      if(nb.level<mylevel) {
        buf_cr.InitWithShallowCopy(pmb->pmr->coarse_ucr_);
      }
    ...
    if(NEW_TH_CON_ENABLED){
      tcns=1, tcne=NTC;
      if(nb.level<mylevel) {
        buf_ntc.InitWithShallowCopy(pmb->pmr->coarse_untc_);
      }
    ...
    if(nb.level==mylevel){
      ...
      if(CR_ENABLED){
        crssize1=LoadCellCenteredBoundaryBufferSameLevel(src_cr, crns, crne, 
                                        &(sbuf[ssize+rssize]), nb, type);
      }
      if(NEW_TH_CON_ENABLED){
        tcssize=LoadCellCenteredBoundaryBufferSameLevel(src_ntc, tcns, tcne, 
                                        &(sbuf[ssize+rssize+crssize1]), nb, type);
      }
      ...
      if(CR_ENABLED){
        crssize1=LoadCellCenteredBoundaryBufferToCoarser(src_cr, crns, crne, 
                                  &(sbuf[ssize+rssize]), buf_cr, nb, type);       
      }
      if(NEW_TH_CON_ENABLED){
        tcssize=LoadCellCenteredBoundaryBufferToCoarser(src_ntc, tcns, tcne, 
                                &(sbuf[ssize+rssize+crssize1]), buf_ntc, nb, type);
      }
    }
    else{
      ...
      if(NEW_TH_CON_ENABLED){
        tcssize=LoadCellCenteredBoundaryBufferToFiner(src_ntc, tcns, tcne, 
                                  &(sbuf[ssize+rssize+crssize1]), nb, type);                                           
      }
    }
    if(nb.rank == Globals::my_rank) {
      std::memcpy(rbuf, sbuf, (ssize+rssize+crssize1+tcssize)*sizeof(Real));
      *flag=BNDRY_ARRIVED;
    }
#ifdef MPI_PARALLEL
    else // MPI
                        ...
                        AthenaArray<Real> &dst_ntc, enum CCBoundaryType type)
{
  ...
  Real *rbuf;
  AthenaArray<Real> cbuf;
  AthenaArray<Real> cbuf_rad, cbuf_cr, cbuf_ntc; 
                 // buffer for radiation, cosmic rays, thermal conduction
  int ns, ne;
  enum BoundaryStatus *flag;
#ifdef MPI_PARALLEL
  MPI_Request *req;
  ...
  for(int n=0; n<pmb->nneighbor; n++) {
    ...
    if(CR_ENABLED){
      flip_cr = flip_across_pole_cr;
    }
    if(NEW_TH_CON_ENABLED){
      flip_ntc = flip_across_pole_ntc;	
    }
#ifdef MPI_PARALLEL
    if(nb.rank!=Globals::my_rank)
    ...
    if(CR_ENABLED){
      if(nb.level<pmb->loc.level) {
        cbuf_cr.InitWithShallowCopy(pmb->pmr->coarse_ucr_);
      }
    }
    if(NEW_TH_CON_ENABLED){
      if(nb.level<pmb->loc.level) {
        cbuf_ntc.InitWithShallowCopy(pmb->pmr->coarse_untc_);
      }
    }
    ...
    if(nb.level==pmb->loc.level){
      ...
      if(CR_ENABLED){
        SetCellCenteredBoundarySameLevel(dst_cr, 0, NCR-1, rbuf, nb, flip_cr, type, p);
      }
      if(NEW_TH_CON_ENABLED){
        SetCellCenteredBoundarySameLevel(dst_ntc, 1, NTC, rbuf, nb, flip_ntc, type, p);
      }
      ...
      if(CR_ENABLED){
        SetCellCenteredBoundaryFromCoarser(0, NCR-1, rbuf, cbuf_cr, nb, flip_cr, type, p);
      }
      if(NEW_TH_CON_ENABLED){
        SetCellCenteredBoundaryFromCoarser(1, NTC, rbuf, cbuf_ntc, nb, flip_ntc, type, p);
      }
    } else {
      int p=0;
      SetCellCenteredBoundaryFromFiner(dst, ns, ne, rbuf, nb, flip, type, p);
      if(RADIATION_ENABLED){
        SetCellCenteredBoundaryFromFiner(dst_rad, 0, 0, rbuf, nb, NULL, RAD, p);
      }
      if(CR_ENABLED){
        SetCellCenteredBoundaryFromFiner(dst_cr, 0, NCR-1, rbuf, nb, flip_cr, type, p);
      }
      if(NEW_TH_CON_ENABLED){
        SetCellCenteredBoundaryFromFiner(dst_ntc, 1, NTC, rbuf, nb, flip_ntc, type, p);
      }
    }
  ...
  if(bflag && (pmb->block_bcs[INNER_X2]==POLAR_BNDRY
    ...
    if(CR_ENABLED)
      PolarSingleCellCentered(dst_cr, 0, NCR-1);   
    if(NEW_TH_CON_ENABLED)
      PolarSingleCellCentered(dst_ntc, 1, NTC);   
  }
