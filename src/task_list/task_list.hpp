...
public:
  ...
  enum TaskStatus RadFluxes(MeshBlock *pmb, int step);
  enum TaskStatus RadIntegrate(MeshBlock *pmb, int step);
  enum TaskStatus RadSourceTerms(MeshBlock *pmb, int step);
  enum TaskStatus RadMomOpacity(MeshBlock *pmb, int step);
  enum TaskStatus RadPhysicalBoundary(MeshBlock *pmb, int step);

  // task functions for cosmic rays
  enum TaskStatus CRFluxes(MeshBlock *pmb, int step);
  enum TaskStatus CRIntegrate(MeshBlock *pmb, int step);
  enum TaskStatus CRSourceTerms(MeshBlock *pmb, int step);
  enum TaskStatus CRVAOpacity(MeshBlock *pmb, int step);
...
// tasks for radiation
  const uint64_t RADPHY_BVAL=1LL<<41;
  const uint64_t CALC_RADFLX=1LL<<4;
  const uint64_t INT_RAD=1LL<<25;
  const uint64_t SRCTERM_RAD=1LL<<21;
  const uint64_t RAD_MOMOPACITY=1L<<42;

// tasks for Cosmic Ray Transport
  const uint64_t CALC_CRFLX=1LL<<43;
  const uint64_t INT_CR=1LL<<44;
  const uint64_t SRCTERM_CR=1LL<<45;
  const uint64_t CR_VAOPACITY=1L<<46;

// tasks for thermal conduction
  const uint64_t INI_TC=1LL<<47; // initialize based on gas quantities
  const uint64_t CALC_TCFLX=1LL<<48;
  const uint64_t INT_TC=1LL<<49;
  const uint64_t SRCTERM_TC=1LL<<50;
  const uint64_t TC_OPACITY=1LL<<51;
