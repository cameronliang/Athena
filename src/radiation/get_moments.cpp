...
void Radiation::CalculateMoment(AthenaArray<Real> &ir_in)
{
  ...
  AthenaArray<Real> i_mom, i_fre_mom;
...
#pragma omp simd
  ...
  if(nfreq > 1){
    i_fre_mom.InitWithShallowCopy(rad_fre_mom);
    for(int n=0; n<13*nfreq; ++n)
      for(int k=0; k<n3z; ++k)
        for(int j=0; j<n2z; ++j)
#pragma omp simd
          for(int i=0; i<n1z; ++i){
            i_fre_mom(n,k,j,i) = 0.0;
          }
  }
...
#pragma omp simd reduction(+:er,frx,fry,frz,prxx,pryy,przz,prxy,prxz,pryz)
          ...
          if(nfreq > 1){
            i_fre_mom(13*ifr+IER,k,j,i) = er;
            i_fre_mom(13*ifr+IFR1,k,j,i) = frx;
            i_fre_mom(13*ifr+IFR2,k,j,i) = fry;
            i_fre_mom(13*ifr+IFR3,k,j,i) = frz;
            i_fre_mom(13*ifr+IPR11,k,j,i) = prxx;
            i_fre_mom(13*ifr+IPR22,k,j,i) = pryy;
            i_fre_mom(13*ifr+IPR33,k,j,i) = przz;
            i_fre_mom(13*ifr+IPR12,k,j,i) = prxy;            
            i_fre_mom(13*ifr+IPR13,k,j,i) = prxz;  
            i_fre_mom(13*ifr+IPR23,k,j,i) = pryz; 
            i_fre_mom(13*ifr+IPR21,k,j,i) = prxy;  
            i_fre_mom(13*ifr+IPR31,k,j,i) = prxz;  
            i_fre_mom(13*ifr+IPR32,k,j,i) = pryz;  
          }// end multiple frequency bands
          er *= wfreq(ifr);
          frx *= wfreq(ifr);
          fry *= wfreq(ifr);
          frz *= wfreq(ifr);
...
#pragma omp simd
  ...
  if(nfreq > 1){
    i_fre_mom.InitWithShallowCopy(rad_fre_mom_cm);
    for(int n=0; n<4*nfreq; ++n)
      for(int k=0; k<n3z; ++k)
        for(int j=0; j<n2z; ++j)
#pragma omp simd
          for(int i=0; i<n1z; ++i){
...
#pragma omp simd reduction(+:numsum,er,frx,fry,frz)
          ...
          //multiply the frequency weight
          if(nfreq > 1){
            i_fre_mom(4*ifr+IER,k,j,i) = er * numsum;
            i_fre_mom(4*ifr+IFR1,k,j,i) = frx * numsum;
            i_fre_mom(4*ifr+IFR2,k,j,i) = fry * numsum;
            i_fre_mom(4*ifr+IFR3,k,j,i) = frz * numsum;
          }
        ...
        if(isotropic){
          ...
          for(int n=0; n<nang; ++n){
            for(int ifr=0; ifr<nfreq; ++ifr){
              sum_sigma_s += sigmas[n*nfreq+ifr] * wfreq(ifr) * weight[n];
              sum_sigma_a += sigmaa[n*nfreq+ifr] * wfreq(ifr) * weight[n];
              sum_planck  += sigmap[n*nfreq*ifr] * wfreq(ifr) * weight[n];
            }
          }
        }
