  ...
  for (int i=root_level; i<=max_level; i++) {
    ...
    for (int j=0; j<nbtotal; j++) {
      if(loclist[j].level==i) {
        SetBlockSizeAndBoundaries(loclist[j], block_size, block_bcs, block_rad_bcs);
...
//----------------------------------------------------------------------------------------
//! \fn void Mesh::EnrollUserRadBoundaryFunction(enum BoundaryFace dir,
//       RadBValFunc_tt my_bc)
//  \brief Enroll a user-defined boundary function for radiation
...
void Mesh::Initialize(int res_flag, ParameterInput *pin)
{
  ...
  do {
    ...
    while (pmb != NULL)  {
      ...
      pcr=pmb->pcr;
      ptc=pmb->phydro->ptc;
      pmb->pbval->SendCellCenteredBoundaryBuffers(phydro->u, prad->ir, pcr->u_cr, 
                                                          ptc->u_tc,  HYDRO_CONS);
    ...
    while (pmb != NULL)  {
      ...
      pcr=pmb->pcr;
      ptc=pmb->phydro->ptc;
      pbval->ReceiveCellCenteredBoundaryBuffersWithWait(phydro->u, prad->ir, 
                                                pcr->u_cr, ptc->u_tc, HYDRO_CONS);
      if (MAGNETIC_FIELDS_ENABLED)
    ...
    if (GENERAL_RELATIVITY && multilevel) {
      ...
      while (pmb != NULL) {
        ...
        pcr=pmb->pcr;
        ptc=pmb->phydro->ptc;
        pmb->pbval->SendCellCenteredBoundaryBuffers(phydro->w, prad->ir,
                                      pcr->u_cr, ptc->u_tc, HYDRO_PRIM);
        pmb=pmb->next;
      }
      ...
      while (pmb != NULL) {
        ...
        pcr=pmb->pcr;
        ptc=pmb->phydro->ptc;
        pbval->ReceiveCellCenteredBoundaryBuffersWithWait(phydro->w, prad->ir, 
                                             pcr->u_cr, ptc->u_tc, HYDRO_PRIM);
    ...
    while (pmb != NULL)  {
      ...
      if(multilevel==true)
        pbval->ProlongateBoundaries(phydro->w, phydro->u, pfield->b, pfield->bcc,
                                       prad->ir, pcr->u_cr, ptc->u_tc, time, 0.0);
      ...
      pbval->ApplyPhysicalBoundaries(phydro->w, phydro->u, pfield->b, pfield->bcc,
                                                     pcr->u_cr, ptc->u_tc, time, 0.0);
      // Apply radiation bounary
      if(RADIATION_ENABLED)
        pbval->ApplyRadPhysicalBoundaries(prad->ir, time, 0.0);
      ...
      if(nloc.level==oloc.level) { // same
        ...
        if(RADIATION_ENABLED){
          BufferUtility::Pack4DData(pb->prad->ir, sendbuf[k], pb->ks, pb->ke,
                    0, n_fre_ang-1, pb->is, pb->ie, pb->js, pb->je, p);
        }
        if(CR_ENABLED){
        ...
        pmr->RestrictCellCenteredValues(pb->phydro->u, pmr->coarse_cons_, 0, NHYDRO-1,
        ...
        if(RADIATION_ENABLED){
          pmr->RestrictCellCenteredValues(pb->prad->ir, pmr->coarse_ir_,
            0, n_fre_ang-1, pb->cis, pb->cie, pb->cjs, pb->cje, pb->cks, pb->cke, RAD);
              
          BufferUtility::Pack4DData(pmr->coarse_ir_, sendbuf[k], pb->cks, pb->cke,
                    0, n_fre_ang-1, pb->cis, pb->cie, pb->cjs, pb->cje, p);
        }
        if(CR_ENABLED){
    ...
    if((ranklist[on]==Globals::my_rank) && (loclist[on].level == newloc[n].level)) {
      ...
      if((loclist[on].level>newloc[n].level)) { // fine to coarse
        for(int ll=0; ll<nlbl; ll++) {
          ...
          pmr->RestrictCellCenteredValues(pob->phydro->u, pmr->coarse_cons_,
               0, NHYDRO-1, pob->cis, pob->cie, pob->cjs, pob->cje,
          ...
          if(RADIATION_ENABLED){
            ...
            pmr->RestrictCellCenteredValues(pob->prad->ir, pmr->coarse_ir_,
                    0, n_fre_ang-1, pob->cis, pob->cie, pob->cjs, pob->cje,
                                            pob->cks, pob->cke, RAD);
          ...
          if(CR_ENABLED){
            ...
            for(int nv=0; nv<NCR; nv++) {
              for(int k=ks, fk=pob->cks; fk<=pob->cke; k++, fk++) {
                for(int j=js, fj=pob->cjs; fj<=pob->cje; j++, fj++) {
                  for(int i=is, fi=pob->cis; fi<=pob->cie; i++, fi++)
                    cr_dst(nv, k, j, i)=cr_src(nv, fk, fj, fi);
            }}}
      ...
      else if((loclist[on].level < newloc[n].level) && (ranklist[on]==Globals::my_rank))
      {
        ...
        if(RADIATION_ENABLED){
          ...
          pmr->ProlongateCellCenteredValues(rad_dst, pmb->prad->ir, 0, n_fre_ang-1,
                                              is, ie, js, je, ks, ke, RAD);
            
        }// End Radiation
        if(CR_ENABLED){

          AthenaArray<Real> &cr_src=pob->pcr->u_cr;
          AthenaArray<Real> &cr_dst=pmr->coarse_ucr_;
      ...
      if(oloc.level==nloc.level) { // same
        ...
        if(RADIATION_ENABLED){
          BufferUtility::Unpack4DData(recvbuf[k], pmr->coarse_ir_,
                            ks, ke, 0, n_fre_ang-1, is, ie, js, je, p);
          pmr->ProlongateCellCenteredValues(pmr->coarse_ir_, pb->prad->ir,
                        0, n_fre_ang-1, is, ie, js, je, ks, ke, RAD);
        }
        if(CR_ENABLED){
