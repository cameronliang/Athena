...
void MeshBlock::InitUserMeshBlockData(ParameterInput *pin)
{
  ...
  if(NEW_TH_CON_ENABLED){
    phydro->ptc->EnrollKappaFunction(FixedConductivity);
  }
  ...
  for(int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=is; i<=ie; ++i) {
        ...
        if(NEW_TH_CON_ENABLED){
            phydro->ptc->u_tc(0,k,j,i) = tgas*rho/(gamma-1.0);
            phydro->ptc->u_tc(1,k,j,i) = -fixedkappa*80.0*x1*exp(-40.0*x1*x1)/phydro->ptc->vmax;
            phydro->ptc->u_tc(2,k,j,i) = 0.0;
            phydro->ptc->u_tc(3,k,j,i) = 0.0;
        }
      }// end i
    }
  }
...
#pragma omp simd
      for(int i=il; i<=iu; ++i){
        ...
        ptc->ntc_kappa(0,k,j,i) = fixedkappa;
