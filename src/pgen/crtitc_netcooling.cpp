...
void ExactCooling_bw(MeshBlock *pmb, const Real time, const Real dt,
  const AthenaArray<Real> &prim, const AthenaArray<Real> &bcc, AthenaArray<Real> &cons);
...
#pragma omp simd
      for(int i=il; i<=iu; ++i){
        ...
        if(t_i > cool_t(0)){
          ...
          Real tmp_Eint = (tnew - t_i) * rho/(gamma_idx - 1.0);  
          ...
          // constant heating option
          //Real cool_rate_coef = tmp_Eint / (dt * gamma_idx - 1.0);  
          //Real const_heatrate =0.042645634594278108; // this is the cooling rate at T=1e5
          //cons(IEN,k,j,i) += const_heatrate * dt /(gamma_idx-1.0);
          
          
          //add a constant heating rate
          if(tnew < 100.0/unit_T){
            //printf("here\n");
            cons(IEN,k,j,i) += dt * heat_coef/(gamma_idx-1.0);
            pmb->user_out_var(1,k,j,i) = dt * heat_coef/(gamma_idx-1.0);
          }else{
