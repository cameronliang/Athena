...
void ATHDF5Output::WriteOutputFile(Mesh *pm, ParameterInput *pin, bool flag)
{
  ...
  if (variable.compare("prim") == 0 or variable.compare("cons") == 0) {
    ...
    if (MAGNETIC_FIELDS_ENABLED)
      num_datasets += 1;
    // Radiation add three data sets: lab moms, co-moving moms, opacity
    ...
    // datasets should be in the same order as loadoutput
    if (RADIATION_ENABLED){
  ...
  if (variable.compare("prim") == 0 || variable.compare("cons") == 0) {
    ...
    else if(variable.compare(0,1,"rad") == 0 && RADIATION_ENABLED)
      std::strncpy(dataset_names[n_dataset++], "rad", max_name_length+1);    
    else if(variable.compare(0,1,"cr") == 0 && CR_ENABLED)
      std::strncpy(dataset_names[n_dataset++], "cr", max_name_length+1);    
    else if(variable.compare(0,1,"tc") == 0 && NEW_TH_CON_ENABLED)
      std::strncpy(dataset_names[n_dataset++], "tc", max_name_length+1);   
    else
